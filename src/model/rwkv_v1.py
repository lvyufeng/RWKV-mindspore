...
def RWKV_Init(module, config): # fancy initialization of all lin & emb layer in the module
    for m in module.modules():
        if not isinstance(m, (nn.Dense, nn.Embedding)):
            continue
...
class RWKV_TimeMix(nn.Cell):
    def __init__(self, config, layer_id):
        ...
        self.key = nn.Dense(config.n_embd, config.n_attn)
        self.value = nn.Dense(config.n_embd, config.n_attn)
        self.receptance = nn.Dense(config.n_embd, config.n_attn)

        # if config.rwkv_tiny_attn > 0:
        #     self.tiny_att = RWKV_TinyAttn(config)

        self.output = nn.Dense(config.n_attn, config.n_embd)
    ...
    def construct(self, x):
        def htu_buhc_bthc(x, y):
            # 1 u t h 1
            x = x.transpose(2, 1, 0).unsqueeze(0).unsqueeze(-1)
            # b u 1 h c
            y = y.unsqueeze(2)
            return (x * y).sum(1)
        ...
        B, T, C = x.size()
        TT = self.ctx_len
        w = ops.pad(self.time_w, (0, TT))
        w = ops.tile(w, [TT])
        w = w[:, :-TT].reshape(-1, TT, 2 * TT - 1)
        w = w[:, :, TT-1:] # w is now a circulant matrix
        w = w[:, :T, :T] * self.time_alpha[:, :, :T] * self.time_beta[:, :T, :]

        x = ops.cat([self.time_shift(x[:, :, :C//2]), x[:, :, C//2:]], axis = -1)
        ...
        k = ops.clamp(k, max=30, min=-60) # clamp extreme values. e^30 = 10^13
        k = ops.exp(k)
        sum_k = ops.cumsum(k, dim=1)

        kv = (k * v).view(B, T, self.n_head, self.head_size)

        wkv = (htu_buhc_bthc(w, kv)).view(B, T, -1)

        rwkv = ops.sigmoid(r) * wkv / sum_k
...
class RWKV_ChannelMix(nn.Cell):
    def __init__(self, config, layer_id):
        ...
        self.key = nn.Dense(config.n_embd, hidden_sz)
        self.value = nn.Dense(config.n_embd, hidden_sz)
        self.weight = nn.Dense(hidden_sz, config.n_embd)
        self.receptance = nn.Dense(config.n_embd, config.n_embd)
    ...
    def construct(self, x):
        ...
        x = ops.cat([self.time_shift(x[:, :, :C//2]), x[:, :, C//2:]], dim = -1)
        k = self.key(x)
        v = self.value(x)
        r = self.receptance(x)
        
        wkv = self.weight(ops.mish(k) * v) # i find mish is a bit better than gelu

        rwkv = ops.sigmoid(r) * wkv

        return rwkv

class RWKV_TinyAttn(nn.Cell): # extra tiny attention
    def __init__(self, config):
        ...
        self.qkv = nn.Dense(config.n_embd, self.d_attn * 3)
        self.out = nn.Dense(self.d_attn, config.n_embd)
    ...
    def construct(self, x, mask):
        ...
        if self.n_head > 1:
            q = q.view(B, T, self.n_head, self.head_size).swapaxes(1, 2)      # (B, T, C) -> (B, nh, T, hs)
            k = k.view(B, T, self.n_head, self.head_size).swapaxes(1, 2)      # (B, T, C) -> (B, nh, T, hs)
            v = v.view(B, T, self.n_head, self.head_size).swapaxes(1, 2)      # (B, T, C) -> (B, nh, T, hs)

        qk = (q @ k.swapaxes(-2, -1)) * (1.0 / math.sqrt(self.head_size))     # (B, nh, T, hs) * (B, nh, hs, T) -> (B, nh, T, T)
        qk = qk.masked_fill(mask == 0, float('-inf'))
        qk = ops.softmax(qk, dim = -1)
        qkv = qk @ v                                                           # (B, nh, T, T) * (B, nh, T, hs) -> (B, nh, T, hs)

        if self.n_head > 1:
            qkv = qkv.swapaxes(1, 2).view(B, T, -1)              # (B, nh, T, hs) -> (B, T, nh, hs) -> (B, T, C)
...
class RotaryEmbedding(nn.Cell):
    def __init__(self, dim, base=10000):
        ...
        self.inv_freq = Parameter(Tensor(inv_freq, ms.float32), 'inv_freq')
